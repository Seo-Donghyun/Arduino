...
// Servo adjustment
#define _DUTY_MAX 1937  // Servo angle: D degree
#define _DUTY_NEU 1500  // Servo angle: 0 degree
#define _DUTY_MIN 1259   // Servo angle: E degree
#define _SERVO_ANGLE_DIFF 300 // Replace with |D - E| degree
#define _SERVO_SPEED 1000 // servo speed limit (unit: degree/second)

#define _BANGBANG_RANGE 220 // 탁구공이 잘 구르지 않아서 좀 증가시켰습니다.
...
void loop() {
  ...
  if(event_dist) {
    ...
    if(dist_target > dist_ema) { // Replace 1 with a proper test expression using dist_target and dist_ema.
      ...
      digitalWrite(PIN_LED, 1); // 예시 영상에서 15.5cm보다 가까운 곳에서 꺼져 있는 것 같아서 LED가 꺼지게 했습니다.
...
//  return (6762.0 / (a_value - 9) - 4.0) * 10.0;
  return 609 - (2.03 * a_value) + (1.7 * pow(10, -3) * pow(a_value, 2));
}
